              ...
              {labels.map((label, index) => (
                <label key={index} onClick={() => handleLabelClick(label, 1)}>
                  <input
                    ...
                    onChange={() => {}}
                  />
                  <span style={{ fontWeight: label === selectedLabel1 ? 'bold' : 'normal' }}>{label}</span>
                  {label === selectedLabel1 && (
                    <Link to="">
                      <img
                        src={del}
                        alt="close"
                        style={{ marginLeft: '10px', width: "20px", height: "auto", display: label === selectedLabel1 ? 'inline-block' : 'none' }}
              ...
              {labelss.map((label, index) => (
                <label key={index} onClick={() => handleLabelClick(label, 2)}>
                  <input
                    ...
                    onChange={() => {}}
                  />
                  <span style={{ fontWeight: label === selectedLabel2 ? 'bold' : 'normal' }}>{label}</span>
                  {label === selectedLabel2 && (
                    <Link to="">
                      <img
                        src={del}
                        alt="close"
                        style={{ marginLeft: '10px', width: "20px", height: "auto", display: label === selectedLabel2 ? 'inline-block' : 'none' }}
                  ...
                  {label === "Youth" && (
                    <span className='m-2'>
                      <Link onClick={handleYouthToggle} style={{ color: 'black' }}>
                        {showYouthChildren ? <AiOutlineMinus/> : <AiOutlinePlus/>} 
                      </Link>
                      {showYouthChildren && (
                        <div>
